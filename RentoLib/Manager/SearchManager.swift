...
struct SearchManager {
    ...
    func searchForResults (_ productFilter: ProductFilter, _ user: User){
        let urlString = "https://torre-test-301909.appspot.com/product"
        ...
        guard let jsonData = try? JSONEncoder().encode(productFilter) else {
                ...
                URLSession.shared.dataTask(with: request) { data, response, error in
                    ...
                    do {
                        ...
                        DispatchQueue.main.async {
                            ...
                            if receivedJson.contains("productId") {
                                ...
                                delegate?.SearchResultsGiven(self, responseMessage, user, JsonResponse)
                            } else{
                                let JsonResponseData = receivedJson.data(using: .utf8)!
                                ...
                                delegate?.SearchFailWithError(errorMessage: responseMessage)
                            }
                        }
                    } catch {
